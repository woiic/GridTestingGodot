...
func draw_weak_grid_line(Coords :Utils.Coordinates , color, C0 = Utils.Coordinates.new()):
	if !Coords:
		return
	for c in MyBoard.find_weak_line(C0,Coords):
		var pts = coords_to_triangle_points(c, MyBoard.TileSize)
		draw_polygon(pts, [color])
